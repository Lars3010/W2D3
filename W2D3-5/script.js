...
// 1
const allHeroNames = superheroes.map((hero) => {
    return hero.name;
});
...
//3
const twoHunderPoundHeroes = superheroes.filter(hero => {
    return hero.weight == 200;
}).map(hero => {
    return hero.name;
});
console.log(twoHunderPoundHeroes);
...
//6
const DCHeroesWeight = superheroes.filter(hero => {
    return hero.publisher == "DC Comics";
}).filter(hero => {
    return hero.weight !== "unknown";
}).reduce((totalSum, item) => {
    console.log(item.weight);
    return (parseInt(item.weight)) + totalSum;
},0);

console.log(DCHeroesWeight);

//7
const MarvelHeroesWeight = superheroes.filter(hero => {
...
const heaviestHero = superheroes.filter(hero => {
    ...
    return hero.weight = parseInt(hero.weight);
}).reduce((a,b) => {
    return Math.max(a,b);
})
